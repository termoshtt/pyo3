    ...
    T: PyObjectAlloc<T> + PyTypeInfo,
{
    ...
    let name = name.expect("Module name/type name must not contain NUL byte")
        .into_raw();
    ...
    if type_object.tp_base != unsafe {
        &ffi::PyBaseObject_Type as *const ffi::PyTypeObject as *mut ffi::PyTypeObject
    } {
